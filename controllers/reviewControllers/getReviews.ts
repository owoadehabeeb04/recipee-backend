...
export const getRecipeReviews = async (req: any, res: any) => {
  try {
    const { recipeId } = req.query;
    const { 
      cursor, // timestamp of last review for infinite scroll
      limit = 10, 
      sortBy = 'rating', // 'rating', 'createdAt', 'helpful'
      sortOrder = 'desc' 
    } = req.query;
    ...
    if (!cursor) {
      const recipe = await RecipeModel.findById(recipeId, 'averageRating totalReviews ratingDistribution');
      aggregation = recipe ? {

...
export const generateShoppingList = async (req: any, res: any) => {
    try {
      const { id } = req.params;
   ...
   */
  export const generateCategorizedShoppingList = async (req: any, res: any) => {
    try {
      const { id } = req.params;
      const userId = req.user._id;
   ...
   */
  export const generatePrintableShoppingList = async (req: any, res: any) => {
    try {
      const { id } = req.params;
      const userId = req.user._id;
      ...
      recipes.forEach(recipe => {
        recipe.ingredients.forEach(ingredient => {
          ...
          if (combinedIngredients.has(key)) {
            ...
            const sameUnitItem = existingItem.items.find((i: any) => i.unit === unit);

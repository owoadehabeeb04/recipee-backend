...
import { embedRecipeDetails, enrichPlanWithRecipes, extractRecipeIds } from './mealplanner';
...
export const createMealPlan = async (req: any, res: any) => {
   ...
   */
  export const duplicateMealPlan = async (req: any, res: any) => {
    try {
      ...
      const sourceMealPlan = await MealPlan.findOne({ _id: id, user: userId });
      ...
      const recipeIds = extractRecipeIds(sourceMealPlan.plan);

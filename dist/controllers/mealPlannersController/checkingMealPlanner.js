...
exports.resetShoppingList = exports.getShoppingListWithStatus = exports.updateShoppingListCheckedItems = void 0;
 ...
 */
const updateShoppingListCheckedItems = async (req, res) => {
    try {
        ...
        // Handle check by category
            ...
            recipes.forEach((recipe) => {
                recipe.ingredients.forEach((ingredient) => {
                    ...
                    const isInCategory = categoryKeywords.length > 0
                        ...
                        : !Object.entries(categories).some(([cat, keywords]) => cat !== 'Miscellaneous' && keywords.some(keyword => lowerName.includes(keyword)));
                    if (isInCategory) {
            ...
            if (checked) {
                ...
                mealPlan.checkedItems = mealPlan.checkedItems.filter((item) => !categoryIngredients.includes(item));
            }
        }
        // Handle specific items
            ...
            if (checked) {
                ...
                const currentSet = new Set(mealPlan.checkedItems);
                normalizedItems.forEach((item) => currentSet.add(item));
                mealPlan.checkedItems = Array.from(currentSet);
            }
            else {
                // Remove items from checked items
                mealPlan.checkedItems = mealPlan.checkedItems.filter((item) => !normalizedItems.includes(item));
            }
        }
        else {
            return res.status(400).json({
...
exports.updateShoppingListCheckedItems = updateShoppingListCheckedItems;
/**
 * Get shopping list with check status
 */
const getShoppingListWithStatus = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user._id;
        // Validate ObjectId
...
exports.getShoppingListWithStatus = getShoppingListWithStatus;
/**
 * Helper function to generate shopping list with checked status
 */
async function generateShoppingListWithStatus(mealPlan) {

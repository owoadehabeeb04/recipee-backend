...
exports.processVoiceMessageStream = exports.processVoiceMessage = void 0;
...
const processVoiceMessage = async (req, res) => {
    try {
        ...
        return await (0, langchainController_1.processChatMessage)(textRequest, res);
    }
    catch (error) {
...
const processVoiceMessageStream = async (req, res) => {
    try {
        const { chatId } = req.params;
        const { transcription } = req.body;
        if (!transcription || transcription.trim().length === 0) {
        ...
        const streamHandler = {
            ...
            sendStatus: (message, usedWebSearch = false) => {
                res.write(`data: ${JSON.stringify({
                    type: 'status',
                    data: { message, usedWebSearch }
                })}\n\n`);
            },
            sendComplete: (aiMessage) => {
                res.write(`data: ${JSON.stringify({
                    type: 'complete',
                    data: { aiMessage }
                })}\n\n`);
...
exports.processVoiceMessageStream = processVoiceMessageStream;
// Custom chat processing function that integrates with your existing logic
async function processChatMessageWithCustomStream(chatId, message, streamHandler, res) {
    try {
        // Get chat and validate
        const chat = await aiChatMessage_1.Chat.findById(chatId);
        if (!chat) {
        ...
        const aiMessage = new aiChatMessage_1.Message({
            chat: chatId,
            role: 'assistant',
            content: '',
            timestamp: new Date()
...
function breakIntoNaturalChunks(content) {
    const chunks = [];
...
function breakIntoCharacterChunks(content, chunkSize = 3) {
    const chunks = [];
    for (let i = 0; i < content.length; i += chunkSize) {

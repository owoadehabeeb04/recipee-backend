...
async function addCreatedAtBulk() {
  try {
    ...
    console.log(
      `Updated ${updateWithUpdatedAt.modifiedCount} users with createdAt from updatedAt`
    );
    ...
    for await (const doc of cursor) {
      if (doc._id) {
        ...
        const timestamp =
          Math.floor(doc._id.getTimestamp().getTime() / 1000) * 1000;
    ...
    console.log(
      `Updated ${count} additional users with createdAt from ObjectId timestamp`
    );
    ...
    if (remaining > 0) {
      console.log(
        `${remaining} users still don't have createdAt, setting to current date`
      );
